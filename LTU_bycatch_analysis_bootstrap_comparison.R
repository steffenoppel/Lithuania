...
## review question 1: does bootstrapping approach stable value?

bootseries<-c(5,10,15,20,25,40,50,60,70,80,100,150,200,250,500,1000,2500,5000,7500,10000,12500,15000,20000,25000,50000)
BPUE_control<-data.frame()
BPUE_kite<-data.frame()
BPUE_night<-data.frame()
BPUE_nightkite<-data.frame()

for (NB in bootseries) {
  
  ## bootstrapping the BPUE samples (NB random draws)
  control.samples <- matrix(sample(controls$BPUE, size = NB * nrow(controls), replace = TRUE),NB, nrow(controls))
  control.statistics <- apply(control.samples, 1, mean)
  BPUE_control<-data.frame(BOOT=NB,type="basic",treatment="control",n=nrow(controls),mean=mean(control.statistics),
                           lcl=quantile(control.statistics,0.025),ucl=quantile(control.statistics,0.975)) %>%
    bind_rows(BPUE_control)
  
  ## adding the bca boot
  BCA <- bcajack(x=controls$BPUE, B = NB, func = mean, verbose = FALSE)
  BPUE_control<-data.frame(BOOT=NB,type="bca",treatment="control",n=nrow(controls),
                           mean=BCA$stats[1,1], #BCA$B.mean[2],
                           lcl=BCA$lims[1,1],ucl=BCA$lims[9,1]) %>%
    bind_rows(BPUE_control)

  
  kite.samples <- matrix(sample(kites$BPUE, size = NB * nrow(kites), replace = TRUE),NB, nrow(kites))
  kite.statistics <- apply(kite.samples, 1, mean)
  BPUE_kite<-data.frame(BOOT=NB,type="basic",treatment="kite",n=nrow(kites),mean=mean(kite.statistics),
                        lcl=quantile(kite.statistics,0.025),ucl=quantile(kite.statistics,0.975)) %>%
    bind_rows(BPUE_kite)
  
  ## adding the bca boot
  BCA <- bcajack(x=kites$BPUE, B = NB, func = mean, verbose = FALSE)
  BPUE_kite<-data.frame(BOOT=NB,type="bca",treatment="kite",n=nrow(kites),
                           mean=BCA$stats[1,1], #BCA$B.mean[2],
                           lcl=BCA$lims[1,1],ucl=BCA$lims[9,1]) %>%
    bind_rows(BPUE_kite)
  ...
  night.samples <- matrix(sample(nights$BPUE, size = NB * nrow(nights), replace = TRUE),NB, nrow(nights))
  night.statistics <- apply(night.samples, 1, mean)
  BPUE_night<-data.frame(BOOT=NB,type="basic",treatment="night",n=nrow(nights),mean=mean(night.statistics),
                         lcl=quantile(night.statistics,0.025),ucl=quantile(night.statistics,0.975)) %>%
    bind_rows(BPUE_night)
  
  ## adding the bca boot
  BCA <- bcajack(x=nights$BPUE, B = NB, func = mean, verbose = FALSE)
  BPUE_night<-data.frame(BOOT=NB,type="bca",treatment="night",n=nrow(nights),
                        mean=BCA$stats[1,1], #BCA$B.mean[2],
                        lcl=BCA$lims[1,1],ucl=BCA$lims[9,1]) %>%
    bind_rows(BPUE_night)
  
  nightkite.samples <- matrix(sample(nightkites$BPUE, size = NB * nrow(nightkites), replace = TRUE),NB, nrow(nightkites))
  nightkite.statistics <- apply(nightkite.samples, 1, mean)
  BPUE_nightkite<-data.frame(BOOT=NB,type="basic",treatment="night_kite",n=nrow(nightkites),mean=mean(nightkite.statistics),
                             lcl=quantile(nightkite.statistics,0.025),ucl=quantile(nightkite.statistics,0.975)) %>%
    bind_rows(BPUE_nightkite)
  
  ## adding the bca boot
  BCA <- bcajack(x=nightkites$BPUE, B = NB, func = mean, verbose = FALSE)
  BPUE_nightkite<-data.frame(BOOT=NB,type="bca",treatment="night_kite",n=nrow(nightkites),
                         mean=BCA$stats[1,1], #BCA$B.mean[2],
                         lcl=BCA$lims[1,1],ucl=BCA$lims[9,1]) %>%
    bind_rows(BPUE_nightkite)
...
bootplot<-bind_rows(BPUE_control,BPUE_kite,BPUE_night,BPUE_nightkite) %>%
  ungroup() %>%
  mutate(BOOT=as.ordered(BOOT)) %>%
  mutate(mean=mean*12322.71,lcl=lcl*12322.71,ucl=ucl*12322.71) %>%
  
  ggplot(aes(x=BOOT, y=mean, linetype=type,col=treatment, fill=treatment, group=interaction(treatment, type)))+

  geom_line(linewidth=1.5)+
  geom_ribbon(aes(x=BOOT, ymin=lcl,ymax=ucl),alpha=0.2)+
  facet_wrap(~ treatment,scales = "fixed")+
  xlab("Number of bootstrap samples") +
  ylab("Mean BPUE") +
  labs(linetype="Bootstrap type",colour = "none",fill="none") %>%
  scale_colour_discrete(guide="none")+
  scale_fill_discrete(guide="none")+
  geom_vline(aes(xintercept=which(bootseries==10000)),colour="red", linewidth=0.5, linetype=2) +
...
ggsave("output/BPUE_bootstrap_assessment.jpg", width=11, height=9)



export_boot<-bind_rows(BPUE_control,BPUE_kite,BPUE_night,BPUE_nightkite) %>%
  mutate(mean=mean*12322.71,lcl=lcl*12322.71,ucl=ucl*12322.71) %>%
  dplyr::filter(BOOT==10000) %>%
  mutate(lcl=if_else(is.na(lcl),0,lcl)) %>%
  mutate(ucl=if_else(is.na(ucl),0,ucl)) %>%
  rename(model=type)
fwrite(export_boot, "output/boot_estimates.csv")
...
#####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~########
#####
#####    BASIC BOOTSTRAP ANALYSIS COMPARING BPUE FOR DIFFERENT SPECIES GROUPS ----------------------------
#####
#####~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~########
...
## SPECIFY THE GROUPS FOR WHICH SUMMARIES ARE SOUGHT
groups<-data.frame(Group=c("Seaducks","Long-tailed Ducks","Cormorants"), col=match(c("SDBPUE","LTBPUE","CORBPUE"),names(data)))

DIFF_SUMMARY<-tibble()
for (sp in 1:dim(groups)[1]){
  
  ## bootstrapping the BPUE samples (10000 random draws)
  control.samples <- matrix(sample(pull(controls,var=groups$col[sp]), size = 10000 * nrow(controls), replace = TRUE),10000, nrow(controls))
  control.statistics <- apply(control.samples, 1, mean)
  BPUE_control<-data.frame(treatment="control",n=nrow(controls),mean=mean(control.statistics),
                           lcl=quantile(control.statistics,0.025),ucl=quantile(control.statistics,0.975))
  
  kite.samples <- matrix(sample(pull(kites,var=groups$col[sp]), size = 10000 * nrow(kites), replace = TRUE),10000, nrow(kites))
  kite.statistics <- apply(kite.samples, 1, mean)
  BPUE_kite<-data.frame(treatment="kite",n=nrow(kites),mean=mean(kite.statistics),
                        lcl=quantile(kite.statistics,0.025),ucl=quantile(kite.statistics,0.975))
  
  night.samples <- matrix(sample(pull(nights,var=groups$col[sp]), size = 10000 * nrow(nights), replace = TRUE),10000, nrow(nights))
  night.statistics <- apply(night.samples, 1, mean)
  BPUE_night<-data.frame(treatment="night",n=nrow(nights),mean=mean(night.statistics),
                         lcl=quantile(night.statistics,0.025),ucl=quantile(night.statistics,0.975))
  
  nightkite.samples <- matrix(sample(pull(nightkites,var=groups$col[sp]), size = 10000 * nrow(nightkites), replace = TRUE),10000, nrow(nightkites))
  nightkite.statistics <- apply(nightkite.samples, 1, mean)
  ...
  ### COMBINE NUMBERS FOR OUTPUT FILE
  DIFF_SUMMARY<-bind_rows(KITE_DIFF_BPUE,NIGHT_DIFF_BPUE,NIGHTKITE_DIFF_BPUE) %>%
    mutate(Group=groups$Group[sp]) %>%
    mutate(mean=mean*12322.71,
           lcl=lcl*12322.71,
           ucl=ucl*12322.71) %>%
    rename(`prop.mean.diff(%)`=mean.1,`prop.lcl.diff(%)`=lcl.1,`prop.ucl.diff(%)`=ucl.1) %>%
    rename(`abs.mean.diff(N)`=mean,`abs.lcl.diff(N)`=lcl,`abs.ucl.diff(N)`=ucl) %>%
    select(Group,treatment,everything()) %>%
    bind_rows(DIFF_SUMMARY)
  
}

fwrite(DIFF_SUMMARY,"output/LIT_mitigation_difference_summary_by_group_2hSS.csv")
